...
describe('websocket', () => {
    ...
    test('should not keep more than 2 connection', () => __awaiter(this, void 0, void 0, function* () {
        const success = jest.fn();
        const fail = jest.fn();
        const complete = jest.fn();
        expect.assertions(8);
        const socketTaskOne = yield Taro.connectSocket({ url: 'wss://localhost:8080', success });
        expect(success.mock.calls[0][0])
            .toEqual({ socketTaskId: 1, socketTask: socketTaskOne, errMsg: 'connectSocket:ok' });
        const socketTaskTwo = yield Taro.connectSocket({ url: 'wss://localhost:8090', success });
        expect(success.mock.calls[1][0])
            .toEqual({ socketTaskId: 2, socketTask: socketTaskTwo, errMsg: 'connectSocket:ok' });
        yield Taro.connectSocket({
    ...
    test('connection关闭的时候会重置总的socketTask数量', () => __awaiter(this, void 0, void 0, function* () {
        const success = jest.fn();
        const fail = jest.fn();
        const complete = jest.fn();
        expect.assertions(7);
        ...
        const socketTaskTwo = yield Taro.connectSocket({ url: 'wss://localhost:8090', success });
        expect(success.mock.calls.length).toBe(2);
        const socketTaskThree = yield Taro.connectSocket({
            url: 'wss://localhost:9080',
            success,
            complete,
            fail
    ...
    test('should work basically', (done) => __awaiter(this, void 0, void 0, function* () {
        ...
        const socketTask = yield Taro.connectSocket({
